...
When the user wants an explicit final review decision, pass `--verdict-options` before any `diff` subcommand. Keep it to 2-4 short labels; decline/blocker-style options should usually require feedback with `!` so the transcript explains why.

```
discuss --verdict-options 'approved:Approve:positive|declined:Decline:negative!' plan.md
```

Use `--verdict-prompt "..."` only when the default finish-review prompt needs project-specific wording. Shell-quote the options because the DSL uses `|` between choices.
...
| POST | `/api/done` | `{verdict: {optionId, feedback?}}` when verdict options are configured; otherwise optional/ignored | Finish the review. Missing body is `400 bad_request`; unknown `optionId` or missing required feedback is `400 validation_error`. |
