...
- **Private-first GitHub PR review** — `discuss pr <full-github-pr-url>` starts a loopback-only PR session and gives the active agent an explicit authenticated-`gh` import protocol. The imported review contains a GFM overview plus one anchored diff file per changed path, retains existing discussion provenance, and keeps every new thread and answer local by default. Finish Review now supports PR actions, editable summary/comments, explicit include controls, an exact GFM confirmation step, grouped inline review publication, resolvable thread replies, and retry-safe agent-mediated publication; nothing reaches GitHub before the reviewer confirms OK. Changed files appear in a GitHub-style expandable folder tree, and diff headers/hunks/additions/deletions use self-contained GitHub-like colors even when Prism's diff grammar is unavailable. Each diff header now carries its addition/deletion totals and has a dedicated whole-file comment button instead of acting as a generic click target, plus a **Viewed** control that records the file, timestamp, and immutable PR head SHA, marks the sidebar entry with an eye indicator, and advances to the next unviewed file. Standalone PR comments and separate OAuth/token configuration remain out of scope. Closes [#51](https://github.com/codesoda/discuss-cli/issues/51).
