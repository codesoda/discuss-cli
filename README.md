...
Everything created during review stays local by default. Diff file headers are not generic thread click targets; use their dedicated speech-bubble control to open an explicit whole-file local comment. Each changed-file header also includes a **Viewed** checkbox; marking it records the server timestamp and current immutable PR head SHA, shows an eye-style viewed indicator in the file tree (not an approval checkmark), and advances to the next unviewed changed file. Viewed progress survives reloads and is included in the final local transcript.
