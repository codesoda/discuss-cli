...
- A rejected verdict on `/api/done` must be a total no-op — it must not emit `session.done`, write history, or signal internal shutdown; validation lives server-side in `validate_done_verdict` so scripted clients get the same policy as the browser modal.
