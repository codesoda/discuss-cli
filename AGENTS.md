...
- Runtime launch resolves `--port`/`--no-open`/`--history-dir`/`--no-save` through `ConfigOverrides`. With no configured port, bind `127.0.0.1:0` via `serve_with_ready` and report the OS-assigned port from a checked `local_addr()`. An explicit port binds exactly and fails fast with no fallback; `DISCUSS_PORT=0` and TOML `port = 0` return `DiscussError::ConfigParseError`.
