...
- Multi-file sidebar paths are grouped by `buildFileTree`/`appendFileTree` into independently collapsible folders. Keep full paths in titles/accessible names, keep active-file ancestors open, aggregate open-thread badges into folders, and preserve the global collapsed icon rail as a flat all-files view. PR diff files expose server-backed viewed state through `/api/pr/files/{id}/viewed`; retain `{fileId, viewedAt, headSha}` in `PrReviewState`, use a non-approval eye treatment in the sidebar, and advance only to the next unviewed diff file in rendered tree order. Diff file headings retain their anchor for explicit whole-file threads but must not act as generic click targets; only the dedicated speech-bubble control opens that editor.
- Private-first PR review contracts live in `src/pr/`, with browser/agent lifecycle handlers in `src/server/pr.rs`. `discuss pr` accepts only a full public-GitHub PR URL and fails before readiness with installation guidance when `gh` is absent. `src/pr/loader.rs` invokes the user's authenticated `gh` plus read-only `git` commands directly, never receives/stores a token, verifies immutable base/head SHAs, and generates exactly one aggregate git diff using the PR command's `--unified <N>` context (default 10) before automatic import. The agent handles only local takes and requested summaries. Only an explicit OK may make `src/pr/publisher.rs` recheck the head and publish the exact confirmed grouped review/thread replies through authenticated `gh`. Include defaults stay false, destinations are server-derived/immutable, ambiguous targets remain unpublished, and a successful direct publication emits `session.done` before archive/shutdown.
