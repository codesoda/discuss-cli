...
- Runtime launch resolves `--port`/`--no-open`/`--history-dir`/`--no-save` through `ConfigOverrides`. With no configured port, bind `127.0.0.1:0` through `startup::start_session`, report the OS-assigned port from a checked `local_addr()`, then serve through `server::serve_on_listener`. An explicit port binds exactly and fails fast with no fallback; `DISCUSS_PORT=0` and TOML `port = 0` return `DiscussError::ConfigParseError`.
- Browser launch/status helpers live in `src/launch.rs`; write `review UI/API: http://127.0.0.1:<port>` to stderr after bind plus `website proxy: <url>` only when a second listener exists, call `open::that` only when `auto_open` remains true, and log browser-open failures with tracing warnings instead of failing the session.
- `startup::start_session` binds every requested listener before emitting `session.started`, writing stderr, or opening the browser. Its payload keys are `url`, `apiBaseUrl`, optional `proxyUrl`, `endpoints`, `agentInstructions`, `mode`, `source_file`, `files_count`, `started_at`, and conditional `git_args`; `url` remains an alias of `apiBaseUrl`.
