...
- UI preferences (theme, Cmd+Enter hint, file-sidebar collapse) are owned by the server: `src/preferences.rs` keeps them in `~/.discuss/preferences.json`, `GET`/`PUT /api/preferences` read and merge them, and `render_page_with_preferences` seeds them into the page above the pre-paint theme bootstrap. A session binds a new port on each launch, so `localStorage` is a same-origin fallback mirror only and never the store of record. `discuss.html` may still touch `localStorage` for those three preferences (gated by the allowlist in `src/template.rs::bundled_template_hydrates_state_from_seed_or_api`), never review state; review state flows through the server seed, REST mutation helpers, SSE updates, and the in-memory `currentState` mirror only, and `STORAGE_KEY = 'discuss-state'` must stay removed.
