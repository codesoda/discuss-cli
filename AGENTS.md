...
- Private-first PR review contracts live in `src/pr/`, with browser/agent lifecycle handlers in `src/server/pr.rs`. `discuss pr` accepts only a full public-GitHub PR URL and fails before readiness with installation guidance when `gh` is absent. `src/pr/loader.rs` invokes the user's authenticated `gh` plus read-only `git` commands directly, never receives/stores a token, verifies immutable base/head SHAs, and generates exactly one aggregate git diff using the PR command's `--unified <N>` context (default 10) before automatic import. The agent handles only requested summaries and confirmed publication; only an explicit OK may emit `pr.publish.requested`. Include defaults stay false, destinations are server-derived/immutable, ambiguous targets remain unpublished, and a successful agent callback emits `session.done` before archive/shutdown.
